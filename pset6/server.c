...
char* indexes(const char* path)
{
    char* pathA = "/index.html";
    char* pathB = "/index.php";
    char* indexA = malloc(strlen(pathA) + strlen(path) + 1);
    strcpy(indexA, path);
    strcat(indexA, pathA);
    char* indexB = malloc(strlen(pathB) + strlen(path) + 1);
    strcpy(indexB, path);
    strcat(indexB, pathB);
    
    if(access(indexA, F_OK) == 1){
        return indexA;
    }
    else if(access(indexB, F_OK) == 1){
        return indexB;
    }
    return NULL;
}
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    char* fileInfo = malloc(sizeof(char)*512);
    
    size_t size = 0;
    for(int i = fgetc(file); i != EOF; i = fgetc(file)){
        if(i >= 512){
            fileInfo = realloc(fileInfo, sizeof(char)*(512*size));
        }
        fileInfo[size] = i;
        size++;
    }
...
{
    char store[strlen(line)+1];
    strcpy(store, line);
    char* method = strtok(store, " ");
    ...
    for(int i = 0;i < strlen(reqTarget); i++){
        if(reqTarget[i] == '"'){
            error(400);
            return false;
        }
    }
    char* HTML = strtok(NULL, " ");
    if(strcmp(HTML, "HTTP/1.1\r\n") != 0){
        error(505);
        return false;
    }
    for(int spot = 0; reqTarget[spot] != '?' && reqTarget[spot] != ' '&& reqTarget[spot] != '\0'; spot++){
        abs_path[spot] = reqTarget[spot];
    }
    return true;
}
